...
if args.function == 'pretrain':
    ...
    pre_trained_text = open(args.pretrain_corpus_path, encoding='utf-8').read()

    #python run.py finetune vanilla wiki.txt --writing_params_path vanila.model.params --finetune_corpus_path birth_places_train.tsv
    ...
    pre_trained_dataset = dataset.CharCorruptionDataset(pre_trained_text, block_size)
    trainer = trainer.Trainer(model, pre_trained_dataset, None, tconf)
    trainer.train()
    ...
    fine_tuning_text = open(args.finetune_corpus_path, encoding='utf-8').read()
    ...
    fine_tuning_dataset = dataset.NameDataset(pretrain_dataset, fine_tuning_text)
    trainer = trainer.Trainer(model, fine_tuning_dataset, None, tconf)
    trainer.train()
