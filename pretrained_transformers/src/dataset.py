...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        random_trunc = random.randint(4, int(self.block_size * 7 / 8))
        inp = self.data[idx]
        my_str = str(inp)
        my_str = my_str[0:random_trunc]

        high_value = random_trunc/4
        random_mask = random.randint(1, 2*int(high_value))#"""int(torch.randint(low=1, high=2*int(random_trunc/4), size=(1,))[0])""" 
        #print(random_len)
        ...
        prefix = my_str[0:len(my_str)//8]
        suffix = my_str[(len(my_str)//8)+random_mask:]

        masked_content = my_str[len(my_str)//8:(len(my_str)//8)+random_mask]

        build_str = prefix+masked_content+suffix
        # print(my_str)
        # print(build_str)
        #print(masked_content)

        temp_str = prefix+self.MASK_CHAR+suffix+self.MASK_CHAR+masked_content

        padding = self.PAD_CHAR*(self.block_size-len(temp_str)-1)

        final_str = temp_str+padding
